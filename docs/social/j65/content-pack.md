# J65 — 3 questions avant la Terminale spé maths

## Source et correction

Les trois questions proviennent de la banque validée du diagnostic SprintMaths,
sans reprendre exactement la sélection J61 :

- `q-124` : `(x² − 4)/(x − 2) = x + 2` pour `x ≠ 2`, par factorisation de `x² − 4` ;
- `q-197` : si `uₙ₊₁ = 0,5uₙ` et `u₀ = 16`, alors `u₄ = 16 × 0,5⁴ = 1` ;
- `q-244` : `(2 ; 3) · (−1 ; 4) = 2 × (−1) + 3 × 4 = 10`.

## Caption TikTok / Instagram

Tu rentres en Terminale spé maths ? Teste trois réflexes sans calculatrice. Le diagnostic complet de 10 questions est gratuit sur SprintMaths, résultat immédiat et aucun email obligatoire.

#Terminale #SpeMaths #Maths #RevisionMaths #SprintMaths

Voix synthétique : SIWIS — Yamagishi, Honnet, Garner et Lazaridis, University of Edinburgh — CC BY 4.0 ; synthèse Piper.

Destination : `sprintmaths.com/diagnostic`

## YouTube Shorts

**Titre** : Prêt pour la Terminale spé maths ? 3 questions

**Description** :

Trois questions flash de calcul, suites et produit scalaire. Le diagnostic complet donne immédiatement ton score et tes priorités, sans email obligatoire.

Fais les 10 questions gratuitement : https://www.sprintmaths.com/diagnostic?utm_source=youtube&utm_medium=social&utm_campaign=diagnostic_social&utm_content=j65_terminal_test

#Shorts #Terminale #SpeMaths #Maths

Voix synthétique : SIWIS — Yamagishi, Honnet, Garner et Lazaridis, University of Edinburgh — CC BY 4.0 ; synthèse Piper.

## Premier commentaire

Tu as fait 3/3 ? Le diagnostic complet est gratuit sur `sprintmaths.com/diagnostic`.

## UTMs

- TikTok : `https://www.sprintmaths.com/diagnostic?utm_source=tiktok&utm_medium=social&utm_campaign=diagnostic_social&utm_content=j65_terminal_test`
- Instagram : `https://www.sprintmaths.com/diagnostic?utm_source=instagram&utm_medium=social&utm_campaign=diagnostic_social&utm_content=j65_terminal_test`
- YouTube : `https://www.sprintmaths.com/diagnostic?utm_source=youtube&utm_medium=social&utm_campaign=diagnostic_social&utm_content=j65_terminal_test`
